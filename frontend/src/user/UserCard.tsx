...
export default function UserCard(props: Props) {
	...
	return (
		<Popup active={props.visible}>
			<Card
				class='fixed w-56 h-auto !p-0 pointer-events-auto overflow-hidden !shadow-lg !my-2'
				ref={ref}
				style={{
					top: `${props.parent.getBoundingClientRect().bottom}px`,
					left: `${
						(props.parent.getBoundingClientRect().left + props.parent.getBoundingClientRect().right) / 2 - 56 * 2
					}px`,
				}}>
				<div class='px-2 py-4'>
					<img
						src='/admin/asset/icon/user-color.svg'
					...
					/>
					<h2 class='mt-4 mb-1 f-head text-xl text-center' title={props.user.username}>
						{props.user.username}
					</h2>
					<h3 class='mb-2 text-center text-sm font-medium text-neutral-300 truncate' title={props.user.id}>
						{props.user.id.slice(0, 12)}
					</h3>
				</div>
