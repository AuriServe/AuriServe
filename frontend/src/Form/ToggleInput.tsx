	...
	return (
		<div ref={fRef} class={merge('flex gap-4 w-full', props.class)} style={props.style}>
			<div class={merge('grow', description === undefined && 'mt-1.5')}>
				<label for={props.id} class='block font-medium leading-none cursor-pointer max-w-xl select-none transition
			...
			<div class={merge('shrink-0 w-10 h-6 relative group isolate', props.toggleLeft && '-order-1')}>
				...
				/>
				<div
					class={merge('absolute inset-0 w-full h-full p-1 transition',
				...
				/>
				<div
					class={merge('absolute w-4 h-4 shadow-sm shadow-neutral-900/50',
