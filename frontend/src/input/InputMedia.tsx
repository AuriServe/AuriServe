		...
		<div class={merge(style.Input, 'flex flex-row-reverse !p-0')} style={props.style} ref={wrapRef}>
			...
			{item && (
				<button
					...
					class='w-12 h-12 p-2 -m-px focus:outline-none'>
			...
			<Popup active={!!search && focused}>
				<SearchableOptionPicker
					...
					fields={['name', 'url']}
					options={media ?? []}
					onSelect={(selected) => handleSet(selected.id)}>
					{({ option, selected }) => (
						<div
							class={merge(
								'flex flex-row flex-grow text-left items-center gap-2',
