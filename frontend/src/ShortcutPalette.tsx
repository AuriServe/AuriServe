	...
	return (
		<Modal active={open} onClose={() => setOpen(false)}
			...
			<form class='w-screen max-w-lg h-[32rem] pb-16' onSubmit={(evt) => handleSubmit(evt)}>
				...
				<div class='relative'>
					<Transition show={results.length === 0 && !noResultsFound} duration={300} invertExit as='div'
					...
					<Transition show={noResultsFound} duration={300} invertExit as='div'
						class='absolute will-change-transform top-0 left-0 w-full flex py-2 justify-center items-center gap-2'
						enter='transition-all duration-150 delay-150'
						exit='transition-all duration-150 delay-0'
						enterFrom='opacity-0 scale-90'
							...
							{results.map((s, i) =>
								<button key={s.title + '_' + i} type='button' onClick={() => handleTriggerAction(i)}
									...
									<div class='flex flex-col self-center'>
										...
										{s.description && <p class='truncate leading-none text-neutral-200 text-sm pt-1'>
											{s.description}
										</p>}
									</div>
								</button>
