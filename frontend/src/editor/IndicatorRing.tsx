...
export default function FocusRing(props: Props) {
	const bounds = {
		...
		width:
			Math.min(
				props.elem.right + FOCUS_RING_POPOUT * 2,
				document.documentElement.scrollWidth - FOCUS_RING_BOUNDARY * 2
			) - props.elem.left,
		height: Math.min(props.elem.bottom + FOCUS_RING_POPOUT * 2, document.documentElement.scrollHeight) - props.elem.top,
	};
	...
	return (
		<div
			style={{
				boxShadow: `inset 0 0 4px 0 rgba(0, 0, 0, ${shadowAlpha}), 0 0 4px 0 rgba(0, 0, 0, ${shadowAlpha})`,
				...bounds,
			}}
			class={merge(
				'box-border absolute rounded-md select-none pointer-events-none border-2 z-50',
