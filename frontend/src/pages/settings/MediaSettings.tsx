			...
			<Card.Body class='py-6 pl-6'>
				...
				<p class='mb-6 text-neutral-200'>
					...
					<span class='font-medium text-neutral-100'>
						{Math.round(((quotas?.storage.used ?? 0) / (quotas?.storage.allocated ?? 0)) * 100)}%{' '}
					</span>
					of the allocated{' '}
					<span class='font-medium text-neutral-100'>{formatBytes(quotas?.storage.allocated ?? 0)}</span> of storage
					space.
				</p>
