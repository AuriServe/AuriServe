	...
	return (
		<div class='w-full'>
			<Button
				...
				exact
				class={merge(
					'group text-neutral-600 dark:text-neutral-100 p-[0.25rem] mb-1 gap-2',
					isActive
						? '!bg-neutral-200/25 dark:!bg-neutral-700 !border-neutral-200 dark:!border-neutral-800'
				...
				<Svg
					...
					size={6}
					class={merge(
						'rounded p-0.5 ml-px transition-colors',
				...
				<Svg
					...
					size={5}
					class={merge(
						'transition opacity-0 group-hover:opacity-50 hover:!opacity-100 !pointer-events-auto',
...
export default function PagesPage() {
	...
	const page = location.pathname.startsWith('/routes/edit/')
		? location.pathname.substr('/routes/edit'.length)
		: undefined;
	...
	return (
		<Page class='flex bg-white dark:bg-neutral-900 !pb-0 min-h-screen'>
			<Title>Pages</Title>
			<div
				class='w-72 p-2.5 h-full bg-neutral-800 flex-shrink-0 relative z-10 rounded-r-xl
				...
				{pages && (
					<div class='animate-fadein-150'>
						...
						<Button
							exact
							class={merge(
								'!w-full text-left text-neutral-600 dark:text-neutral-200 p-[0.25rem]',
