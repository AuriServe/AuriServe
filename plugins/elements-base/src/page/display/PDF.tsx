...
function RawPDF(props: Props) {
	const path = props.path?.startsWith('data:') ? props.path :
		toPath(props.path ?? getMedia(props.media ?? -1)?.path ?? 'unknown');
...
const PDF = hydrate<Props>(identifier, RawPDF, (props) => {
	const path = props.inline ? `data:application/pdf;base64,${fs.readFileSync(getMedia(props.media!)!.path, 'base64')}` :
		toPath(props.path ?? getMedia(props.media!)?.path);
	return {
		path,
