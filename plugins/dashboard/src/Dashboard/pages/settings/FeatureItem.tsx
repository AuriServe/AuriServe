	...
	return (
		<div class='relative flex items-end w-full aspect-[3/2]'>
			<button
				...
				{coverPath ? (
					<img
						...
						alt=''
						role='presentation'
						class={merge('absolute inset-0 bg-gradient-to-br object-cover', !enabled && 'grayscale')}
					/>
				) : (
					<div
						class='absolute inset-0 bg-gradient-to-br from-accent-600 to-accent-800'
						style={{ filter: !enabled ? 'sepia(100%) hue-rotate(180deg) saturate(220%) brightness(60%)' : '' }}
					/>
				)}
				<div
					class={merge(
						'absolute w-full p-3 pt-8 bottom-0 top-auto',
					...
					<div class='relative'>
						<p
							...
							class='top-0 left-0 w-full text-sm line-clamp-1 leading-5 text-black dark:text-white
								transition-all opacity-75 group-hocus:opacity-0'>
							{description}
						</p>
						<p
							ref={(elem) => {
							...
							transition-all opacity-0 group-hocus:opacity-75'>
							{description}
						</p>
					</div>
				</div>
			...
			<button class='group absolute top-0 right-0 w-10 h-10 p-2 cursor-pointer !outline-none' onClick={handleToggle}>
				...
				<div
					class={merge(
						'w-full h-full rounded-full shadow border-2 transition',
						enabled && 'border-white bg-white',
						!enabled && 'border-white/75 scale-90'
					)}>
