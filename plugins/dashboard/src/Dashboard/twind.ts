import { escape, dom, css, inject, colorFromTheme } from 'twind';

import { setup } from '@twind/tailwind';
// import colors from '@twind/tailwind/colors';
// import defaultTheme from '@twind/tailwind/defaultTheme';
...
setup(
	{
		...
		variants: [
			...
			[
				'((group|peer)(-[^-]+)?)-hocus',
				({ 1: $1 }: { 1: string }, { hash }: any) =>
					`.${escape(hash($1))}:where(:hover,:focus-visible) &`,
			],
			[
				'peer-focus',
				(_: any, { hash }: any) => `.${escape(hash('peer'))}:focus-visible ~ &`,
			],
			[
				'peer-placeholder-shown',
				(_: any, { hash }: any) => `.${escape(hash('peer'))}:placeholder-shown ~ &`,
			],
			...
			[
				'test',
				(_: any, { hash }: any) =>
					`.${escape(hash('peer'))}:is(:placeholder-shown:not(:focus)) ~ &`,
			],
		],
		theme: {
			extend: {
...
inject(css`
	@font-face {
		font-weight: 100 900;
		font-family: 'Roboto';
		src: url('/admin/asset/font_roboto.ttf') format('truetype');
	}
	...
	:root {
		${Object.entries(grayColors.light)
			.map(([label, color]) => `--theme-gray-${label}: ${color};`)
			.join('\n')}
	}
	...
	${Object.entries(themeColors)
		.map(
			...
			${Object.entries(colors)
				.map(([label, color]) => `--theme-accent-${label}: ${color};`)
				.join('\n')}
		}`
		)
		.join('\n')}
