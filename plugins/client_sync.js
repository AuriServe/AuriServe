...
function getAverageColor(img, precision = 5) {
  let data;
...
toCompute.forEach(img => {
  ...
  for (let i = 0; i < upCount; i++) elem = elem.parentElement;

  elem.style.setProperty('--palette-true', color__WEBPACK_IMPORTED_MODULE_0___default()(color).string().substring(4).replace(/\)$/, ''));
...
expandElems.forEach(elem => {
  const text = elem;
  let i = 0;
  text.innerHTML = text.innerText.split(' ').map(w => `<span>${w.split('').map(c => `<span style='--i:${i++}'>${c}</span>`).join('')}</span>`).join('');
});
})();

window.__ASP_INDIEWEB = __webpack_exports__;
/******/ })()
;
//# sourceMappingURL=client_sync.js.map