...
const PAGE_TEMPLATE = fss.readFileSync(path.join(path.dirname(__dirname), 'views', 'page.html')).toString();

// The error page template, containing $MARKERS$ for page content.
const ERROR_TEMPLATE = fss.readFileSync(path.join(path.dirname(__dirname), 'views', 'error.html')).toString();
