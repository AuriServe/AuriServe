...
const DatePicker = compat_1.forwardRef((props, ref) => {
    const [month, setMonth] = hooks_1.useState(() => new Date().getMonth());
    const [year, setYear] = hooks_1.useState(() => new Date().getFullYear());
    hooks_1.useEffect(() => {
        setMonth(props.value.getMonth());
        setYear(props.value.getFullYear());
    }, [props.value]);
    const handleNavigate = (months) => {
    ...
    for (let i = 1; true; i += 7) {
        ...
        for (let j = 0; j < 7; j++) {
            ...
            const isSelectedDate = date.getDate() === props.value.getDate() &&
            ...
            const rep = [date.getDate(), date.getMonth(), date.getFullYear()];
            days.push(<button tabIndex={-1} class={dayClasses} onClick={() => setDate(...rep)}>{date.getDate()}</button>);
            date.setDate(date.getDate() + 1);
        }
    }
    return (<div class={('DatePicker ' + (props.parent ? 'Absolute' : '')).trim()} ref={ref} style={style}>
