...
require("./SearchableOptionPicker.sass");
const InputLabel_1 = tslib_1.__importDefault(require("./InputLabel"));
function sortOptions(query, a, b) {
...
function SearchableOptionPicker(props) {
    const [index, setIndex] = hooks_1.useState(0);
    const optionsArray = hooks_1.useMemo(() => Array.isArray(props.options) ?
    ...
    hooks_1.useEffect(() => setIndex(0), [query]);
    hooks_1.useEffect(() => {
        const handleArrowSelect = (evt) => {
            ...
            if (filteredOptions.length === 0)
                return;
            if (evt.key === 'ArrowUp')
                setIndex((index) => (index <= 0 ? filteredOptions.length : index) - 1);
            else if (evt.key === 'ArrowDown')
                setIndex((index) => (index + 1) % filteredOptions.length);
            else if (evt.key === 'Enter')
                props.setSelected(filteredOptions[index]);
        };
    ...
    return (<div class='SearchableOptionPicker' style={style} onMouseDown={evt => { evt.preventDefault(); evt.stopPropagation(); }}>
			{query && <InputLabel_1.default label={`Search results for '${query}'`}/>}
			<ul class='SearchableOptionPicker-Items'>
				{filteredOptions.map((option, ind) => <li class={('SearchableOptionPicker-Item ' + (index === ind ? 'Focused' : '')).trim()} key={ind} onClick={() => props.setSelected(option)}>
                ...
                <span class='SearchableOptionPicker-DefaultItem'>
								{Array.isArray(props.options) ? option : props.options[option] || as_common_1.Format.fileNameToName(option)}
							</span>}
					</button>
				</li>)}
...
exports.default = SearchableOptionPicker;
