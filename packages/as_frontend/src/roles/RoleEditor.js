...
function RoleEditor(props) {
    ...
    const handleNameChange = (name) => {
        props.setRole(Object.assign(Object.assign({}, JSON.parse(JSON.stringify(props.role))), { name }));
    };
    const handleToggleAbility = (ability) => {
        const abilityStr = ability;
        let role = Object.assign(JSON.parse(JSON.stringify(props.role)));
        if (role.abilities.filter((f) => f === abilityStr).length)
            role.abilities = role.abilities.filter((f) => f !== abilityStr);
        else
            role.abilities.push(abilityStr);
        props.setRole(role);
    };
    const renderAbilitySwitch = (ability, title, description, sep) => {
        return (<Preact.Fragment>
				<Input.Annotation title={title} description={description}>
					<Input.Checkbox setValue={() => handleToggleAbility(ability)} value={props.role.abilities.filter(f => f === ability).length > 0}/>
				</Input.Annotation>
				{sep !== false && <hr class='RoleEditor-Separator'/>}
			</Preact.Fragment>);
    };
    return (<div class='RoleEditor'>
			<Input.Label label='Role Name'>
				<Input.Text style={props.role.color && { color: as_common_1.Color.convert(props.role.color).toHex() }} value={props.role.name} setValue={handleNameChange}/>
			</Input.Label>
            ...
            .map(c => <button style={{ backgroundColor: c }} onClick={() => handleSetColor(as_common_1.Color.convert(c).toHSVA())} title={c}>
							{props.role.color && as_common_1.Color.convert(props.role.color).toHex() === c &&
			...
			{renderAbilitySwitch('ADMINISTRATOR', 'Administrator', 'Bypass ability checks and role heirarchy. Use with caution.')}
			...
			{renderAbilitySwitch('RESET_USER_PASSWORD', 'Reset user Passwords', 'Send password reset emails, and directly set user passwords.')}
