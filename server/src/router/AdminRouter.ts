...
export default class AdminRouter extends Router {
	...
	init() {
		...
		this.router.post(
			'/graphql',
			authRoute,
			Router.safeRoute(async (req, res) =>
		...
		this.router.post(
			...
			Router.safeRoute(async (req, res) => {
				...
				if (typeof replace !== 'string') throw 'Request is missing required data.';
		...
		this.router.get(
			'/themes/cover/:identifier.jpg',
			authRoute,
			Router.safeRoute(async (req, res) =>
				res.sendFile(
					path.join(this.dataPath, 'themes', req.params.identifier, 'cover.jpg')
				)
			)
		);
		...
		this.router.get(
			'/plugins/cover/:identifier.jpg',
			authRoute,
			Router.safeRoute((req, res) =>
				res.sendFile(
					path.join(this.dataPath, 'plugins', req.params.identifier, 'cover.jpg')
				)
			)
		);
		...
		this.router.use(
			'/script',
			Express.static(
				path.join(path.dirname(path.dirname(__dirname)), 'interface', 'dist')
			)
		);
		this.router.use(
			'/asset',
			Express.static(path.join(path.dirname(path.dirname(__dirname)), 'interface', 'res'))
		);
		...
		this.router.get('/client.js', async (req, res) => {
			try {
				...
				const error = typeof e === 'string' ? e : 'Internal server error.';
				res.send(`/* ${error} */`);
			}
		});
		...
		this.router.get('(/*)?', async (_, res) => {
			const html = (
				...
				.replace(
					'$PLUGINS$',
					`<script id='plugins' type='application/json'>${JSON.stringify({
						pluginScripts: this.plugins
							.listEnabled()
							.filter((p) => p.manifest.entry.client.script)
							.map((p) => `${p.manifest.identifier}/${p.manifest.entry.client.script}`),
						pluginStyles: this.plugins
							.listEnabled()
							.filter((p) => p.manifest.entry.client.style)
							.map((p) => `${p.manifest.identifier}/${p.manifest.entry.client.style}`),
					})}</script>`
				)
				.replace(
					'$THEMES$',
					`<script id='themes' type='application/json'>${JSON.stringify({
						themes: this.themes.listEnabled().map((t) => t.identifier),
					})}</script>`
				)
				.replace(
					'$DEBUG$',
